...
import { X, Minus, Image as ImageIcon, Send, File, Paperclip } from 'lucide-react';
...
const MiniChatBox = ({ conversation, positionIndex }) => {
  const { closeMiniChat, socket, onlineUsers } = useChat();
        ...
        <>
                  ...
                  <div className={`flex gap-2 max-w-[85%] ${isMine ? 'flex-row-reverse' : 'flex-row'}`}>
                    {!isMine && (
                       <div className="w-6 shrink-0 flex items-end pb-1">
                          {showAvatar && <Avatar src={msg.sender.profilePicture} fallback={msg.sender.firstName[0]} size="xs" />}
                       </div>
                    )}

                    <div className={`flex flex-col gap-1 ${isMine ? 'items-end' : 'items-start'}`}>
                      {msg.text && (
                        <div className={`px-3 py-1.5 rounded-2xl text-[14px] ${isMine ? 'bg-brown-600 text-white rounded-br-sm' : 'bg-brown-100 text-brown-900 rounded-bl-sm'}`}>
                          {msg.text}
                        </div>
                      )}
                        ...
                        <div className="flex flex-wrap gap-1 mt-1 max-w-[200px]">
                          {msg.media.map((item, i) => (
                            <div key={i} className="rounded-lg overflow-hidden border border-brown-200 bg-white">
                              {item.type === 'image' ? (
                                <img src={item.url} alt="attachment" className="max-h-24 object-contain" />
                              ) : item.type === 'video' ? (
                                <video src={item.url} controls className="max-h-24 object-contain" />
                              ) : (
                                <a href={item.url} target="_blank" rel="noreferrer" className="block p-1 text-[10px] text-blue-600 underline">
                                  Attachment
                                </a>
                              )}
                            </div>
                      ...
                      {msg.linkPreview && (
                        <a href={msg.linkPreview.url} target="_blank" rel="noopener noreferrer" className="block w-48 border border-brown-200 rounded-lg overflow-hidden bg-white mt-1 hover:opacity-90">
                          {msg.linkPreview.image && <img src={msg.linkPreview.image} alt="Preview" className="w-full h-24 object-cover" />}
                          <div className="p-2">
                            <p className="text-xs font-semibold text-brown-900 truncate">{msg.linkPreview.title}</p>
                            <p className="text-[10px] text-brown-500 line-clamp-2 mt-0.5">{msg.linkPreview.description}</p>
                          </div>
                        </a>
                      )}
                    </div>
          ...
          <div className="p-2 border-t border-brown-100 bg-white flex flex-col gap-1">
            ...
            <form onSubmit={handleSend} className="flex items-end gap-2 bg-brown-50 rounded-2xl p-1 border border-brown-200">
              ...
              />
              <button type="button" onClick={() => fileInputRef.current?.click()} className="p-1.5 text-brown-400 hover:text-brown-600 rounded-full transition">
                <Paperclip size={18} />
              </button>
              <textarea
                value={text}
